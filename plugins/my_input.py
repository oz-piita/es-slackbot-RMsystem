...
def readsheet(id):
    ...
    arr_N = df_need.drop(df_need.index[range(len(lessons), len(
        df_need.index))]).replace("", 0).astype("int").values
    ...
    # 該当日を切り出し
    df_res = df_down[[0, 1, 3*date_index +
                      2, 3*date_index+3, 3*date_index+4]]
    df_res = df_res.rename(
        columns={3*date_index+2: 2, 3*date_index+3: 3, 3*date_index+4: 4})
    ...
    df_res = df_res.set_index(
        0)[lastans - datetime.timedelta(days=30):lastans]
    ...
    df_attend = pd.DataFrame(columns=range(
        len(df_res.index)), index=range(len(df_lesson.index)))
